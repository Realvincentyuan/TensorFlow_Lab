...
train_data_gen = image_gen.flow_from_directory(directory=train_dir,target_size=(IMG_SHAPE, IMG_SHAPE),shuffle=True)
...
image_gen = tf.keras.preprocessing.image.ImageDataGenerator(rescale=1./255, rotation_range=45, zoom_range=0.5, horizontal_flip=True, width_shift_range=0.15, height_shift_range=0.15)

train_data_gen = image_gen.flow_from_directory(directory=train_dir,target_size=(IMG_SHAPE, IMG_SHAPE),shuffle=True, class_mode='sparse')
...
val_data_gen = image_gen.flow_from_directory(directory=val_dir, target_size=(IMG_SHAPE, IMG_SHAPE),  class_mode='sparse')
...
history = model.fit_generator(train_data_gen, epochs=epochs, validation_data=val_data_gen, callbacks=[early_stopping])
...
class ResidualUnit(tf.keras.layers.Layer):
    def __init__(self, filters, strides=1, activation='Relu', **kwargs):
        ...
        self.main_layers = [
            tf.keras.layers.Conv2D(filters, 3, strides=strides, padding='same', use_bias=True),
            tf.keras.layers.BatchNormalization(),
            self.activation,
            tf.keras.layers.Conv2D(filters, 3, strides=1, padding='same', use_bias=True),
            tf.keras.layers.BatchNormalization(),
        ]
        self.skip_layers = []
        if strides > 1:
            self.skip_layers = [
                tf.keras.layers.Conv2D(filters, 1, strides=strides, padding='same', use_bias=True),
                tf.keras.layers.BatchNormalization(),
            ]
...
# Build Resnet-34 resnet_model with Kera sequential API
resnet_model = tf.keras.models.Sequential()
resnet_model.add(tf.keras.layers.Conv2D(64, 7, strides=2, input_shape=[IMG_SHAPE, IMG_SHAPE, 3], padding='same', use_bias=False))
resnet_model.add(tf.keras.layers.BatchNormalization())
resnet_model.add(tf.keras.layers.ReLU())
...
for filters in [64] * 3 + [128] * 4 + [256] * 6 + [512] * 3:
    ...
    resnet_model.add(ResidualUnit(filters=filters, strides=strides, activation='relu'))
    pre_filters = filters
...
history_v2 = resnet_model.fit_generator(train_data_gen, epochs=epochs, validation_data=val_data_gen, callbacks=[early_stopping])
